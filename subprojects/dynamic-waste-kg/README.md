...
3. [三层知识图谱运行与联动指南](docs/knowledge_graph_runtime_zh.md)
4. [数据集处理与感知流水线](docs/dataset_and_perception_pipeline_zh.md)
...
当前 `dynamic-waste-agent` 已提供 `WasteKgRuntimeAdapter`，可将 Agent 的只读候选查询和四类受控写入接到本子项目的内存 `KnowledgeGraph`。`Neo4jGraphMirror` 可在每次 `kg_writer` 提交后，先确保 Schema，再以单个受控数据事务把三层图谱镜像到 Neo4j，并同步刷新 UI 快照；Agent 仍不能提交任意 Cypher。
