...
- `conf >= 0.85`：采用 YOLO 类别假设，但仍由知识图谱检查是否允许处理；
- `0.40 <= conf < 0.85`：调用 VLM 提取视觉属性并做一致性校验；
- `conf < 0.40`：不强行分类，写入 `unknown` 并触发人工复核；
- 类别之间容易混淆时，调用 VLM；
- VLM 与 YOLO 冲突时，不自动改类，进入 `uncertain/unknown`。

### 推荐阈值

- `high_confidence >= 0.85`：采用 YOLO 假设；
- `0.40 ~ 0.85`：进入 VLM 属性一致性校验；
- `< 0.40`：进入 `unknown` 和人工复核。
