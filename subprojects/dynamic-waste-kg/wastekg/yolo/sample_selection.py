...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weights", default="outputs/yolo_runs/paper_e1/yolo11s_seg_waste11_grouped_e50_v1/weights/best.pt")
...
def iter_ground_truth(root: Path, names: dict[int, str], split: str, max_images: int = 0) -> Iterable[GroundTruthObject]:
    ...
    for image_path in image_paths:
        ...
        for line in label_path.read_text(encoding="utf-8").splitlines():
            # 先从 YOLO 分割标注恢复 GT bbox，后续只在同类目标之间匹配预测框。
...
def predict_by_image(
    ...
    for start in range(0, len(image_paths), batch_size):
        # 按 batch 调 YOLO，避免一次性加载过多图像造成 8GB 显存压力。
...
def main() -> int:
    ...
    for obj in gt_objects:
        ...
        for pred in predictions.get(obj.image_path, []):
            # 代表样本必须先同类匹配，再比较 IoU，避免把高置信误检选进论文样例。
