...
- 所以模型不能、也不应宣称识别 `asbestos_suspect`。
- `glass` 后续应通过补充数据、大模型复核或人工确认处理。
...
| `glass`、`unknown`/高风险疑点 | 当前不依赖 YOLO 自动确认，必须复核或人工确认 |
| 位置和深度可靠 | 可用于规划候选 |
...
4. 补充 `glass` 数据；
5. 高风险材料不建议依赖普通视觉数据直接判断，应进入 `unknown`/人工复核，优先人工确认；
...
- 数据集格式可用；
- 10 类已有正样本可以训练；
- `glass` 需要后续补数据或复核；高风险材料应通过 `unknown`/人工复核机制处理；
