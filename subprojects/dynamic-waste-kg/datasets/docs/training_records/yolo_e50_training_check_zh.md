...
- `glass` 当时没有正样本，因此不应宣称该轮模型能可靠识别玻璃。
- `asbestos_suspect` 不属于当前 YOLO 训练类别。
...
高风险材料不应直接作为普通 YOLO 确定类别训练。

理由：

- 石棉是否存在通常不能只靠 RGB 图像可靠确认；
- 外观上可能与石膏板、保温材料、纤维水泥板、旧瓦片等混淆；
- 如果模型把普通材料误判为高风险材料，会影响任务效率；
- 如果模型漏判真实高风险材料，会带来安全风险；
- 对机器人系统来说，这类情况更适合进入 `unknown` 和人工复核，而不是新增确定视觉类别。

建议路线：

1. YOLO 只负责检测 11 个明确视觉类别；
2. 在线多模态大模型或人工复核只提供风险线索和人工复核建议；
3. 一旦进入 `unknown`/人工复核状态，任务规划默认不让机械臂自动夹取；
4. 图谱中设置 `risk_level=high` 或 `unknown`、`needs_llm_review=True`、`auto_processable=False`、`handling_mode=human_review` 或 `human_only`。
...
- 高风险材料：不要训练成确定视觉类别，先做 `unknown`/人工复核机制；
